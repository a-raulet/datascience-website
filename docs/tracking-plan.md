...
| `newsletter_signup` | Soumission du formulaire Buttondown « Signaux Japon » (home, `/ressources/signaux-japon`, pied des analyses Radar) | `lang` (l'URL de la page est capturée automatiquement par Umami) | Indicateur avancé du funnel : lecture → inscription → familiarité → appel. **Implémentation différente des autres events** : attributs `data-umami-event` / `data-umami-event-*` inline sur le bouton submit (gérés par le script Umami directement, pas par `tracking.js`), car le formulaire est statique. Les gardes DNT et `data-domains` s'appliquent côté Umami. Note : l'event mesure le clic submit, pas la confirmation double opt-in côté Buttondown. |
